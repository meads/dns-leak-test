...
declare uuid_to_dispatch_events_for=''
...
while [ $uuid_chosen -eq 1 ] ; do
  # banner
  
  if [ $selection_error -eq 0 ] ; then
    tput setaf 1
  ...
  if [ $selection_error -eq 1 ] && [ ${#uuid_to_dispatch_events_for} -gt 0 ] ; then
    uuid_chosen=0
    device_uuid_directory="/etc/NetworkManager/dispatcher.d/${uuid_to_dispatch_events_for}"
